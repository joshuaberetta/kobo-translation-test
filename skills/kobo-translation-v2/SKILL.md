...
This skill is maintained through source files in `sources/`:

| File | Purpose | Update When |
|------|---------|-------------|
| `glossary.xlsx` | All terminology tables | Adding/changing terms |
| `style-guide.md` | General translation guidelines | Changing style rules |
| `workflow-rules.md` | Workflow, checklists, decision tree | Changing processes |
| `language-rules.md` | French/Spanish/Arabic specific rules | Adding language-specific guidance |

**To update terminology:**
1. Edit `sources/glossary.xlsx`
2. Run `python scripts/update_skill.py`
3. Reference files in `references/` are regenerated

**To update guidelines:**
1. Edit the appropriate source markdown file
2. Update this SKILL.md as needed (workflow/rules are also documented here)

**Scripts available:**
- `scripts/update_skill.py` - Validate sources and regenerate references
- `scripts/validate_sources.py` - Check source file integrity
- `scripts/sync_and_update.py` - Import external source files and update
