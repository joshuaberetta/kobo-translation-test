...
<p class="note">
  <strong>Remarque :</strong> Cet article se concentre sur l'ajout de branchement conditionnel dans <a href="https://support.kobotoolbox.org/getting_started_xlsform.html">XLSForm</a>. Pour en savoir plus sur l'ajout de branchement conditionnel dans l'interface de création de formulaires KoboToolbox (KoboToolbox Formbuilder), consultez <a href="https://support.kobotoolbox.org/skip_logic.html">Ajouter un branchement conditionnel dans l'interface de création de formulaires</a>.
    <br><br>
Pour une pratique concrète du branchement conditionnel dans XLSForm, consultez le <a href="https://academy.kobotoolbox.org/courses/xlsform-fundamentals">Cours sur les fondamentaux XLSForm</a> de KoboToolbox Academy.
</p>

## Ajouter des conditions de branchement conditionnel à des questions individuelles

Le branchement conditionnel utilise le [référencement de questions](https://support.kobotoolbox.org/form_logic_xls.html#question-referencing) pour afficher uniquement les questions pertinentes pour le ou la répondant(e) en fonction des réponses précédentes. La question utilisée pour définir la logique de pertinence est appelée la **question de référence**.
...
Les questions sans réponse sont traitées comme des chaînes vides, notées par deux apostrophes simples `''`. Les conditions de branchement conditionnel suivantes peuvent être utilisées :

| Condition de branchement conditionnel | Description |
|:---------------------|:-------------|
| `${question_de_référence} != ''` | Afficher uniquement si `question_de_référence` a reçu une réponse (n'est pas vide). |
| `${question_de_référence} = ''` | Afficher uniquement si `question_de_référence` n'a pas reçu de réponse (est vide). |

**Feuille survey**

| type         | name      | label                | relevant             |
|:--------------|:-----------|:---------------------|:---------------------|
| text          | raison_adhesion | Pourquoi avez-vous adhéré ?    |                      |
| select_one yn | benefices   | Constatez-vous des bénéfices ? | ${raison_adhesion} != '' |

## Ajouter des conditions de branchement conditionnel à des groupes de questions

Le branchement conditionnel peut être appliqué à des groupes de questions ainsi qu'à des questions individuelles. Appliquer un branchement conditionnel à un groupe affichera ou masquera toutes les questions au sein de ce groupe en fonction des réponses précédentes. 
...
| type         | name        | label                                         | relevant            |
|:-------------|:------------|:---------------------------------------------|:-------------------|
| select_one yn | adhesion     | Avez-vous adhéré à l'association ?                |                     |
| begin_group  | association | Participation à l'association                    | ${adhesion} = 'oui'  |
| date         | date_adhesion | Quand avez-vous adhéré à l'association ?           |                     |
| select_one yn | vote      | Avez-vous déjà voté lors d'une élection de l'association ? |                     |
| end_group    |             |                                              |                     |