...
Les tableaux ci-dessous répertorient les apparences de questions courantes par type de question et indiquent lesquelles sont disponibles dans les formulaires web Enketo et KoboCollect.

### Questions à choix multiple
Les questions à choix multiple permettent aux répondant(e)s de [choisir parmi des options prédéfinies](https://support.kobotoolbox.org/question_types_xls.html#select-question-types).

| Apparence | Description | Compatibilité |
| :--- | :--- | :--- |
| `minimal` | Affiche les choix dans un menu déroulant. | Enketo et KoboCollect |
| `compact` | Affiche les choix côte à côte avec un espacement minimal et sans cases de sélection. | Enketo et KoboCollect |
| `label` | Affiche les libellés des choix sans les cases de sélection. | Enketo et KoboCollect |
| `list-nolabel` | Affiche les cases de sélection sans les libellés. | Enketo et KoboCollect |
| `autocomplete` | Ajoute une barre de recherche en haut de la liste d'options. | Enketo et KoboCollect (combiner avec l'apparence minimal) |
| `horizontal` | Affiche les choix de réponse horizontalement en colonnes. | Enketo uniquement |
| `horizontal-compact` | Affiche les choix de réponse horizontalement, avec un espacement minimal et sans cases de sélection. | Enketo uniquement |
...
| `quickcompact` | Affiche les choix côte à côte avec un espacement minimal et sans cases de sélection, et fait avancer automatiquement à la question suivante après la sélection d'une réponse (`select_one` uniquement). | KoboCollect uniquement |
...
| `map` | Affiche une carte pour sélectionner des options. Nécessite de <a href="https://support.kobotoolbox.org/select_from_map_xls.html">définir des coordonnées GPS</a> dans l'onglet `choices` (`select_one` uniquement). | KoboCollect uniquement |
| `quick map` | Affiche une carte pour sélectionner des options, en enregistrant automatiquement le premier emplacement sélectionné sans afficher d'informations supplémentaires. Nécessite de <a href="https://support.kobotoolbox.org/select_from_map_xls.html">définir des coordonnées GPS</a> dans l'onglet `choices` (`select_one` uniquement). | KoboCollect uniquement |

<p class="note">
  <b>Note :</b> Les apparences pour les questions <code>select_one</code> et <code>select_multiple</code> peuvent également être utilisées pour les questions <a href="https://support.kobotoolbox.org/select_from_file_xls.html">sélectionner à partir d'un fichier</a>.
</p>

### Questions de type chiffre et décimale
...
### Question de type intervalle
Les questions de type intervalle sont utilisées pour [sélectionner des valeurs dans une plage spécifiée](https://support.kobotoolbox.org/question_types_xls.html#numeric-question-types).
...
Les questions de type texte permettent aux utilisateur(rice)s de [collecter des réponses ouvertes](https://support.kobotoolbox.org/question_types_xls.html#text-and-note-question-types).
...
| `masked` | Masque le texte saisi par le/la répondant(e) (par exemple, un mot de passe ou des informations confidentielles). | KoboCollect uniquement |


### Question de type date
Les questions de type date sont utilisées pour [capturer des dates de calendrier spécifiques](https://support.kobotoolbox.org/question_types_xls.html#date-and-time-question-types).

| Apparence | Description | Compatibilité |
| :--- | :--- | :--- |
| `month-year` | Capture un mois et une année. | Enketo et KoboCollect |
| `year` | Capture uniquement une année. | Enketo et KoboCollect |
| `no-calendar` | Affiche un sélecteur rotatif pour sélectionner le jour, le mois et l'année, au lieu du sélecteur de calendrier par défaut. | KoboCollect uniquement |
...
| Apparence | Description | Compatibilité |
| :--- | :--- | :--- |
| `maps` | Affiche une carte permettant aux utilisateur(rice)s de visualiser l'emplacement qui est enregistré automatiquement (`geopoint` uniquement). | KoboCollect uniquement (inclus dans l'apparence Enketo par défaut)  |
...
### Questions de type image
Les questions de type image permettent aux utilisateur(rice)s de [télécharger ou d'enregistrer des images](https://support.kobotoolbox.org/question_types_xls.html#media-question-types) directement dans leurs formulaires.

| Apparence | Description | Compatibilité |
| :--- | :--- | :--- |
| `signature` | Permet aux utilisateur(rice)s de dessiner leur signature. | Enketo et KoboCollect |
| `draw` | Permet aux utilisateur(rice)s de faire des croquis ou de créer des dessins. | Enketo et KoboCollect |
| `annotate` | Permet aux utilisateur(rice)s d'annoter une image en dessinant ou en écrivant dessus. | Enketo et KoboCollect |
| `new` | Invite les utilisateur(rice)s à prendre une nouvelle photo à l'aide de l'appareil photo de l'appareil (pas de téléchargement de fichier). | KoboCollect uniquement |
| `new-front` | Invite les utilisateur(rice)s à prendre une nouvelle photo à l'aide de l'appareil photo frontal de l'appareil. | KoboCollect uniquement |


### Questions de type code-barres
Les questions de type code-barres permettent aux utilisateur(rice)s de [scanner un code QR](https://support.kobotoolbox.org/question_types_xls.html#media-question-types) à l'aide de l'appareil photo de l'appareil dans KoboCollect.
