...
Les groupes répétés dans XLSForm vous permettent de poser le même ensemble de questions plusieurs fois au sein d'un formulaire. Cela est particulièrement utile pour collecter des informations similaires sur plusieurs personnes, objets ou événements.
...
- Ajouter une [logique de saut](https://support.kobotoolbox.org/grouping_questions_xls.html#applying-skip-logic-to-question-groups) aux groupes répétés dans la colonne **relevant**.
...
| type | name | label | repeat_count |
| :--- | :--- | :--- | :--- |
| integer | participants | Nombre total de participant(e)s à la formation | |
...
En dehors d'un groupe répété, vous pouvez récupérer des données du groupe répété pour les utiliser dans la logique de formulaire ou les libellés de questions :
...
3. La question **calculate** stocke la valeur récupérée, que vous pouvez ensuite utiliser dans la logique de formulaire ou les libellés de questions.
