...
| note | instructions | Répondez aux questions ci-dessous pour chaque participant. | | **${number} = 1** |
...
2. Incluez l'une des formules répertoriées ci-dessous dans la colonne **calculation**.
...
| `join('; ', ${question-name})` | Répertorie toutes les réponses à une question donnée à l'intérieur d'un groupe répété, séparées par un point-virgule. |
| `indexed-repeat(${question-name}, ${repeat-name}, n)` | Récupère la valeur pour `${question-name}`, dans le groupe répété appelé `${repeat-name}`, dans la n<sup>ième</sup> répétition. |
