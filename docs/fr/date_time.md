...
Il existe 3 types de questions de date et heure dans KoboToolbox : « Date », « Heure » et « Date et heure ».

La question de type « Date » sert à recueillir des valeurs de date, par exemple pour demander une date de naissance, etc. Dans KoboCollect et dans les formulaires web Enketo, un sélecteur de date de style calendrier s'affiche pour sélectionner la date.

La question de type « Heure » sert à recueillir des valeurs d'heure, par exemple pour une question comme « À quelle heure partez-vous au travail ? ». Dans KoboCollect et Enketo, un sélecteur d'heure s'affiche où l'utilisateur(rice) peut sélectionner sa réponse.
...
- Dans l'interface de création de formulaires, appuyez sur le bouton <i class="k-icon k-icon-plus"></i> pour ajouter une nouvelle question
- Saisissez le texte de la question, par exemple « Quelle est votre date de naissance ? », puis appuyez sur **AJOUTER UNE QUESTION** ou appuyez sur ENTRÉE sur votre clavier
- Choisissez le type de question

![Ajouter les questions](images/date_time/adding.gif)
...
Lors de l'ajout de la question de type « Date » dans l'interface de création de formulaires, vous pouvez choisir parmi plusieurs options d'affichage (sous les paramètres de la question). Les apparences modifient la façon dont la question s'affiche dans les formulaires web et dans KoboCollect.

Pour la question de type « Date », vous pouvez contrôler la façon dont le calendrier grégorien par défaut s'affiche en choisissant parmi les options « month-year » (mois année), « year » (année) et « no-calendar » (sans calendrier). En plus de ces options, vous pouvez également modifier le style de calendrier pour utiliser des calendriers non grégoriens pris en charge.

![Ajouter des apparences avancées](images/date_time/advanced_appearance.png)
...
_\* Ces options doivent être saisies manuellement dans l'interface de création de formulaires après avoir sélectionné « other » (autre)._

### Ajouter des apparences personnalisées pour les questions de date dans XLSForm
...
## Utiliser les questions de date et heure dans une logique personnalisée
