...
Cet article couvre les options de questions les plus courantes et comment les ajouter à votre XLSForm, y compris les indices de questions, les questions obligatoires, les réponses par défaut et les paramètres de questions.

<p class="note">
  <strong>Note :</strong> Cet article se concentre sur la définition des options de questions dans <a href="https://support.kobotoolbox.org/getting_started_xlsform.html">XLSForm</a>. Pour en savoir plus sur les options de questions dans l'interface de création de formulaires KoboToolbox, consultez <a href="https://support.kobotoolbox.org/question_options.html">Options de questions dans le Formbuilder</a>.
<br><br>
Pour une pratique concrète des options de questions dans XLSForm, consultez le <a href="https://academy.kobotoolbox.org/courses/xlsform-fundamentals">cours XLSForm Fundamentals</a> de la KoboToolbox Academy.
</p>
...
- Les **indices de question de base** sont utilisés pour fournir des informations supplémentaires aux répondant(rice)s ou aux enquêteur(rice)s directement dans le formulaire. Ils sont toujours visibles et affichés sous le libellé de la question.
- Les **instructions supplémentaires** sont utilisées pour fournir des informations supplémentaires lors du développement du formulaire, de la formation des enquêteur(rice)s ou de la collecte de données. Elles ne sont pas affichées par défaut.

### Ajouter des indices de questions dans XLSForm

Pour ajouter un **indice de question de base** dans XLSForm :
...
<p class="note">
<strong>Note :</strong> Les indices de questions peuvent également être traduits dans plusieurs langues. Pour plus d'informations sur la traduction de formulaires, consultez <a class="reference" href="https://support.kobotoolbox.org/language_xls.html">Ajouter des traductions dans XLSForm</a>.
</p>

### Afficher les instructions supplémentaires dans KoboCollect

Dans les formulaires web Enketo, les instructions supplémentaires apparaissent dans une section **Plus de détails** repliable. Dans KoboCollect, elles sont masquées par défaut, mais vous pouvez [modifier les paramètres de votre projet](https://support.kobotoolbox.org/kobocollect_settings.html#form-management-settings) pour toujours les afficher ou les afficher dans une section repliable.
...
<p class="note">
<strong>Note :</strong> Les conditions de logique de saut ont la priorité sur les paramètres <strong>obligatoires</strong>, ce qui signifie que si une question obligatoire est masquée par une logique de saut, il n'est plus obligatoire d'y répondre.
</p>
...
2. Saisissez le texte que vous souhaitez afficher lorsque les utilisateur(rice)s laissent une question obligatoire vide.

**onglet survey**

| type | name | label | required | required_message |
| :--- | :--- | :--- | :--- | :--- |
| select_one education | education_level | Quel est le niveau d'éducation le plus élevé que vous avez atteint ? | TRUE | |
| integer | age | Quel est votre âge ? | TRUE | Veuillez répondre à cette question avant de continuer. |
| survey |

<p class="note">
<strong>Note :</strong> Une logique de formulaire personnalisée peut être utilisée pour rendre une question obligatoire ou facultative en fonction d'une réponse précédente. Pour en savoir plus sur la logique d'obligation conditionnelle, consultez <a class="reference" href="https://support.kobotoolbox.org/required_logic_xls.html">Ajouter une logique d'obligation conditionnelle dans XLSForm</a>.
</p>
...
La réponse par défaut sera enregistrée comme réponse finale lors de la soumission du formulaire **sauf si elle est modifiée par le/la répondant(e)** pendant la collecte de données. Pour empêcher les répondant(e)s de modifier une réponse par défaut, ajoutez une colonne **read_only** et définissez-la sur **TRUE**.

<p class="note">
<strong>Note :</strong> Bien que les réponses par défaut puissent rendre la collecte de données plus efficace en pré-remplissant le formulaire avec des réponses attendues ou courantes, elles risquent également d'introduire des biais ou des erreurs dans les données et doivent être utilisées avec prudence.
</p>
...
| select_one marital_options | marital_status | Quel est votre statut matrimonial ? | married |
...
| select_multiple | **Nom(s)** du/des choix, séparés par un **espace** s'il y en a plusieurs |
...
2. Dans la colonne `trigger`, saisissez la question qui activera le calcul.
    - En général, il s'agirait de la même question référencée dans la colonne `calculation`, de sorte que tout changement apporté à la question déclencheur mettra également à jour la réponse par défaut.
...
| range | phone | Veuillez sélectionner un nombre entre 1 et 5. | start=1 end=5 step=1 |
| survey |
...
| max-pixels=480 | image | Limite le nombre maximal de pixels pour une photo, afin de réduire la taille du fichier image et d'améliorer la vitesse de téléchargement |
| quality=low | audio | Capture un enregistrement audio de qualité inférieure |
| quality=voice-only | audio | Capture l'enregistrement audio de la plus faible qualité |

## Options de questions supplémentaires

Les XLSForms peuvent inclure des colonnes supplémentaires dans l'onglet survey pour des formulaires et des fonctionnalités plus avancés. Quelques-unes sont énumérées ci-dessous.
