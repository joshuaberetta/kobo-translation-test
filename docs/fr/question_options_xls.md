# Utiliser les options de questions dans XLSForm
...
- Les **indices de base** sont utilisés pour fournir des informations supplémentaires aux répondants ou aux enquêteurs directement dans le formulaire. Ils sont toujours visibles et affichés sous le libellé de la question.
- Les **instructions supplémentaires** sont utilisées pour fournir des informations supplémentaires lors du développement du formulaire, de la formation des enquêteurs ou de la collecte de données. Elles ne sont pas affichées par défaut.
...
| :--- | :--- | :--- | :--- | :--- |
| integer | height | Quelle est votre taille ? | En centimètres | Si le répondant ne connaît pas sa taille, saisissez 0 |
| survey |

<p class="note">
<strong>Note :</strong> Les indices de questions peuvent également être traduits en plusieurs langues. Pour plus d'informations sur la traduction de formulaires, consultez <a class="reference" href="https://support.kobotoolbox.org/language_xls.html">Ajouter des traductions dans XLSForm</a>.
</p>

### Afficher les instructions supplémentaires dans KoboCollect

Dans les formulaires web Enketo, les instructions supplémentaires apparaissent dans une section pliable **Plus de détails**. Dans KoboCollect, elles sont masquées par défaut, mais vous pouvez [modifier les paramètres de votre projet](kobocollect_settings.md#form-management-settings) pour toujours les afficher ou les afficher dans une section pliable.
...
Par défaut, les questions d'un formulaire sont facultatives. Définir une question comme **obligatoire** rend obligatoire pour le répondant d'y répondre. Cela peut être utile pour garantir que les soumissions sont complètes et éviter les données manquantes.
...
Si un répondant ne répond pas à une question obligatoire, il ne pourra pas passer à la page suivante ou soumettre le formulaire. Le message obligatoire par défaut « Ce champ est obligatoire » sera affiché.
...
Une **réponse par défaut** remplit une question avec une réponse prédéfinie basée sur une réponse courante ou attendue. La réponse par défaut peut être fixe ou [déterminée dynamiquement](question_options_xls.md#définir-des-réponses-par-défaut-dynamiques) en fonction de la réponse à une question précédente.

La réponse par défaut sera enregistrée comme réponse finale lorsque le formulaire sera soumis **sauf si elle est modifiée par le répondant** pendant la collecte de données. Pour empêcher les répondants de modifier une réponse par défaut, ajoutez une colonne **read-only** et définissez-la sur **TRUE**.
...
2. Saisissez la réponse par défaut, en suivant le [format approprié](question_options_xls.md#format-de-réponse-par-défaut) pour le type de question.
...
| select_one | **Nom** du choix (tel que défini dans la feuille de calcul choices) |
| select_multiple | **Nom(s)** du choix, séparés par un **espace** s'il y en a plusieurs |
...
2. Saisissez le [paramètre](question_options_xls.md#paramètres-de-questions-courants) approprié pour votre type de question.
...
| start=1 end=5 step=1 | range | Définit la valeur minimale, la valeur maximale et l'intervalle entre les nombres |
