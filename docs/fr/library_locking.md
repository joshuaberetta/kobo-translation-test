...
Le verrouillage de bibliothèque contrôle quelle partie d'un formulaire **peut être modifiée** lorsqu'un projet est créé à partir d'un modèle de bibliothèque. Les restrictions sont définies dans votre XLSForm avant de téléverser le formulaire.
...
- **Feuille kobo--locking-profiles :** Pour définir des profils qui regroupent des restrictions associées.
...
| <code>question_settings_edit</code>     | Empêche la modification des paramètres de question, y compris le nom de la question. Cela n'inclut pas le branchement conditionnel ni les critères de validation. |
...
<details>
  ...
  <ul>
  <li>Les restrictions sont appliquées uniquement dans l'<strong>interface de création de formulaires.</strong> Si le XLSForm est téléchargé et modifié directement dans un tableur, les restrictions n'empêchent pas les modifications.</li>
  ...
  <li>Seuls les formulaires et modèles prennent en charge le verrouillage. Si vous téléversez un XLSForm verrouillé en tant que question ou bloc, le verrouillage est ignoré.</li>
