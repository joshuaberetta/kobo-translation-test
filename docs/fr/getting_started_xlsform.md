...
Lors de la création de formulaires d'enquête avec KoboToolbox, vous pouvez créer votre formulaire avec l'interface de création de formulaires KoboToolbox (KoboToolbox Formbuilder) ou en utilisant XLSForm. XLSForm est très efficace pour créer des formulaires de base et avancés dans un format convivial.

<p class="note">
  Pour plus d'informations sur XLSForm, consultez <a class="reference" href="https://support.kobotoolbox.org/edit_forms_excel.html">Introduction à XLSForm</a>. Pour une introduction complète au développement de formulaires à l'aide de XLSForm, nous vous recommandons <a class="reference" href="https://academy.kobotoolbox.org/courses/xlsform-fundamentals">le cours en ligne à votre rythme sur les fondamentaux XLSForm de KoboToolbox Academy</a>.
</p>

Cet article explique comment configurer un XLSForm à l'aide de Microsoft Excel ou d'autres tableurs, notamment :

- Configuration de la structure de base de votre XLSForm
- Ajout de questions et de choix d'options
- Ajout de paramètres de formulaire et de colonnes facultatives
- Téléchargement et prévisualisation de votre XLSForm dans KoboToolbox

<p class="note">
  <b>Remarque :</b> Certaines fonctionnalités XLSForm ne sont actuellement pas disponibles dans l'interface de création de formulaires, mais les formulaires KoboToolbox peuvent être téléchargés, modifiés en XLSForm et <a class="reference" href="https://support.kobotoolbox.org/xlsform_with_kobotoolbox.html">téléversés à nouveau dans KoboToolbox</a>.
</p>

## Configuration d'un XLSForm
...
<p class="note">
Pour commencer avec XLSForm, téléchargez un exemple de XLSForm <a class="reference" href="https://support.kobotoolbox.org/_static/files/getting_started_xlsform/sample_xlsform.xlsx">ici</a>.
</p>
...
| Nom de colonne | Description |
| --- | --- |
| list_name | Définit l'identifiant unique d'une liste de choix d'options |
...
4. Si vous ajoutez des **questions de type sélection** (`select_one`, `select_multiple` ou `rank`) : dans la colonne `type`, après le type de question, ajoutez un espace et saisissez le nom de la liste de choix.
...
## Ajout de choix d'options

Pour les questions de type sélection, les choix d'options sont ajoutés dans la feuille de calcul **choices**. Pour ajouter des choix d'options dans XLSForm :
...
## Ajout de colonnes facultatives à la feuille de calcul survey
...
Si votre XLSForm contient une erreur, un message d'erreur apparaîtra, indiquant généralement la ligne, la question ou l'expression exacte où se trouve le problème. Après avoir corrigé l'erreur dans votre XLSForm, vous devrez téléverser à nouveau le fichier.

<p class="note">
Pour savoir comment télécharger un XLSForm depuis KoboToolbox, importer votre XLSForm via une URL et utiliser KoboToolbox pour valider et tester votre XLSForm, consultez <a class="reference" href="https://support.kobotoolbox.org/xlsform_with_kobotoolbox.html">Utilisation de XLSForm avec KoboToolbox</a>.
</p>