...
| `stroke`          | Couleur HEX pour la ligne de tracé géographique ou le contour du polygone géographique. |
| `stroke-width`    | Largeur de la ligne de tracé géographique ou du contour du polygone géographique (par exemple, 5 ou 6.5). |
...
Dans KoboCollect, l'ajout d'une **question de type sélection** avec une apparence **map** ou **quick map** permet aux utilisatrices et utilisateurs de choisir une option directement à partir d'une carte plutôt qu'à partir d'une liste. Lorsque la carte s'ouvre, elle se centre sur l'emplacement actuel de l'appareil. Les boutons à droite permettent aux utilisatrices et utilisateurs de recentrer sur leur emplacement ou d'afficher tous les points de carte disponibles.
