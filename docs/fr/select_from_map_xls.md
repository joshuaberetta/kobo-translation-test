...
Avec les types de questions **select_one** et **select_one_from_file**, vous pouvez configurer votre XLSForm pour permettre aux utilisatrices et utilisateurs de sélectionner un choix directement à partir d'une carte plutôt que d'une liste. Lorsque la carte s'ouvre, elle affiche tous les points, lignes ou polygones disponibles en fonction des choix fournis. Les utilisatrices et utilisateurs peuvent ensuite appuyer sur un élément de la carte pour enregistrer leur sélection. Cette fonctionnalité est disponible **uniquement dans KoboCollect.**
...
Lors de la saisie des coordonnées GPS dans la feuille `choices` ou dans un fichier CSV ou XML externe, le format suivant doit être utilisé : `longitude latitude altitude accuracy`.
...
| `stroke`          | Couleur HEX pour la ligne de géotrace ou le contour du polygone de géoforme. |
...
Dans KoboCollect, l'ajout d'une **question de type sélection** avec une apparence **map** ou **quick map** permet aux utilisatrices et utilisateurs de choisir une option directement à partir d'une carte plutôt que d'une liste. Lorsque la carte s'ouvre, elle se centre sur l'emplacement actuel de l'appareil. Les boutons sur la droite permettent aux utilisatrices et utilisateurs de recentrer sur leur emplacement ou d'afficher tous les points de carte disponibles.

Les choix de points sont affichés sous forme de marqueurs de carte. Appuyer sur un marqueur augmente sa taille.
Les choix de lignes et de polygones sont affichés sous forme de contours rouges, avec des polygones ombrés en rouge. Les utilisatrices et utilisateurs peuvent appuyer sur une ligne ou un polygone pour le sélectionner. Lorsqu'un emplacement est sélectionné, ses [propriétés](https://support.kobotoolbox.org/select_from_map_xls.html#adding-choice-properties) apparaissent en bas de l'écran, sauf si l'apparence **quick map** est utilisée.
