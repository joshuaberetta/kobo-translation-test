...
1. Dans la feuille `settings`, ajoutez une colonne **default_language**.
...
Pour configurer la feuille `survey` :

1. Renommez la colonne **label** en utilisant le format **label::langue (code)**.
    - Par exemple : `label::English (en)`.
2. Si votre formulaire comprend des colonnes `hint`, `required_message`, `constraint_message` ou `media` dans la feuille `survey`, renommez les colonnes existantes en utilisant le format **nom_colonne::langue (code)**.
...
Enfin, pour configurer la feuille `choices`, renommez la colonne **label** en utilisant le format **label::langue (code)**.
...
Pour ajouter des traductions à la feuille `survey` :
...
2. Si votre formulaire comprend des colonnes `hint`, `required_message`, `constraint_message` ou `media` dans la feuille `survey`, configurez les colonnes de traduction correspondantes en utilisant le format **nom_colonne::langue (code)**.
...
Pour ajouter des traductions à la feuille `choices` :
1. Ajoutez une nouvelle colonne **label** pour chaque langue de traduction en utilisant le format **label::langue (code)**.
    - Par exemple : `label::Spanish (es)`.
2. Saisissez la traduction de chaque libellé de choix dans la colonne de traduction correspondante.
3. Si votre feuille `choices` comprend des colonnes de médias, configurez les colonnes de traduction correspondantes en utilisant le format **nom_colonne::langue (code)**.
...
### Utiliser les fonctionnalités des tableurs pour les traductions en masse

XLSForm facilite la traduction des éléments de formulaire en masse, au lieu de saisir les traductions une par une. Par exemple, vous pouvez copier une colonne entière dans un système de traduction pour une traduction en masse, puis coller la colonne dans votre XLSForm. Si vous utilisez Google Sheets pour créer votre XLSForm, vous pouvez utiliser la formule `GOOGLETRANSLATE()` pour automatiser le processus de traduction.

Les traductions automatiques doivent toujours être révisées et validées par une personne parlant couramment la langue pour garantir l'exactitude, la pertinence culturelle et le contexte approprié. Cette étape permet de maintenir la qualité et la fiabilité de votre contenu traduit.
